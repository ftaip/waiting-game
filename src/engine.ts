...
function spriteSize(sprite: readonly (readonly number[])[]): { w: number; h: number } {
...
export function tick(state: GameState, dt: number, input: GameInput): GameState {
  ...
  next.bubbles = state.bubbles.map((b) => {
    let nx = b.x - moveDist * 0.4 + b.drift * dt;
  ...
  if (next.distance > 0 && next.distance / 1000 >= 60 * effectiveSpeed) {
    // ~60 seconds of survival at current speed
    if (state.distance < 60 * 1000 * BASE_SPEED) {
      // Use a rough wall-clock estimate via "time alive" derived from distance/speed
    }
  }
  // simpler: track achievements based on score / pearls / near-misses
...
function decayShake(shake: ScreenShake, dt: number): ScreenShake {
